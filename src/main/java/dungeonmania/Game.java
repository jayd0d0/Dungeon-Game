...
public class Game {
    ...
    private int countEnemySlained = 0;
    ...
    public Game build(String buildable) throws InvalidActionException {
        ...
        } else if (buildable.equals("midnight_armour") && !player.noZombiesAround(map)) {
            throw new InvalidActionException(
                    String.format("%s cannot be built, there are zombies on the map", buildable));
        }
    ...
    public <T extends Entity> List<T> getMapEntities(Class<T> entityClass) {
        return map.getEntities(entityClass);
    }

    public void removeBuildableItem(InventoryItem item) {
        player.remove(item);
    }
    ...
    public void spawnZombies(Game game, ZombieToastSpawner spawner) {
        entityFactory.spawnZombie(game, spawner);
    }

    public void spawnSpiders(Game game) {
        entityFactory.spawnSpider(game);
    }

    public int getCountEnemySlained() {
        return countEnemySlained;
    }

    public void setCountEnemySlained(int countEnemySlained) {
        this.countEnemySlained = countEnemySlained;
    }

    public boolean sceptreInInv() {
