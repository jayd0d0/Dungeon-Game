...
public class Inventory {
    ...
    public List<String> getBuildables() {
        ...
        if ((wood >= 1 || arrows >= 2) && (keys >= 1 || treasure >= 1 || sunStones > 1) && sunStones >= 1) {
            result.add("sceptre");
        }
    ...
    public InventoryItem checkBuildCriteria(Player p, boolean remove, boolean forceShield, EntityFactory factory,
        ...
        if (wood.size() >= 2 && (treasure.size() >= 1 || keys.size() >= 1) && forceShield) {
            if (remove) {
                items.remove(wood.get(0));
            ...
            return factory.buildShield();
                ...
                && (keys.size() >= 1 || treasure.size() >= 1 || sunStones.size() > 1) && (sunStones.size() >= 1)
                && !forceShield) {
            if (remove) {
                if (wood.size() >= 1) {
                    items.remove(wood.get(0));
        ...
        } else if (swords.size() >= 1 && sunStones.size() >= 1 && !forceShield && noZombies) {
            if (remove) {
    ...
    public void useInventoryWeapon(Game game) {
        getWeapon().use(game);
    }
