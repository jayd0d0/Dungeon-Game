...
public class Player extends Entity implements Battleable {
    ...
    public <T> List<T> getPlayerInventory(Class<T> entityClass) {
        return inventory.getEntities(entityClass);
    }

    public <T extends InventoryItem> T getFirstInventory(Class<T> itemType) {
        return inventory.getFirst(itemType);
    }

    public void removeCollectableItem(InventoryItem item) {
        inventory.remove(item);
    }

    public void useWeapon(Game game) {
        inventory.useInventoryWeapon(game);
    }
    ...
    public boolean noZombiesAround(GameMap map) {
