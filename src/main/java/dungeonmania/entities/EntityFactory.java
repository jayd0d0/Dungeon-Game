...
import dungeonmania.entities.buildables.BuildableProperties;
...
public class EntityFactory {
    ...
    public void spawnZombie(Game game, ZombieToastSpawner spawner) {
        ...
        if (spawnInterval == 0 || (tick + 1) % spawnInterval != 0)
            return;
        List<Position> pos = spawner.getCardinalAdjPositions();
    ...
    public Bow buildBow() {
        ...
        BuildableProperties bowProperties = new BuildableProperties(bowDurability, 0, 0, 0, 0);
        return (Bow) BuildableFactory.createBuildable("Bow", bowProperties);
    }
    ...
    public Shield buildShield() {
        ...
        BuildableProperties shieldProperties = new BuildableProperties(shieldDurability, shieldDefence, 0, 0, 0);
        return (Shield) BuildableFactory.createBuildable("Shield", shieldProperties);
    }

    public Sceptre buildSceptre() {
        int mindControlDuration = config.optInt("mind_control_duration");
        BuildableProperties sceptreProperties = new BuildableProperties(0, 0, mindControlDuration, 0, 0);
        return (Sceptre) BuildableFactory.createBuildable("Sceptre", sceptreProperties);
    }

    public MidnightArmour buildMidnightArmour() {
        double armourAttack = config.optInt("midnight_armour_attack");
        double armourDefence = config.optInt("midnight_armour_defence");
        BuildableProperties armourProperties = new BuildableProperties(0, 0, 0, armourAttack, armourDefence);
        return (MidnightArmour) BuildableFactory.createBuildable("MidnightArmour", armourProperties);
    }
